...
ACTION eosblog::setconfig(string blogname, string nickname, string version, string metadata) {
  ...
  config_singleton.nickname = nickname;
...
/*
ACTION eosblog::hi(name from, string message) {
  require_auth(from);
